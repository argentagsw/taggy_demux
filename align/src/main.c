#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <dirent.h>
#include <ctype.h>
#include <assert.h>
#include <limits.h>
#ifdef PARALLEL
#include <pthread.h>
#endif
#include "DB.h"
//#include "util.h"
//#include "strutil.h"
#include "args.h"
#include "process.h"
#include "align.h"

static void *process_thread(void *arg);

int main (int argc, char *argv[]){
  globals_t         *g;
  files_t           *files;
  argopts_t          p;
  int                i;
  #ifdef PARALLEL
  pthread_t         *threads;
  #endif

  arg_parse(argc, argv, &p);
  if(p.VERBOSE > 4) arg_print(&p);

  g = Malloc(p.NUM_THREADS * sizeof(globals_t),"Global(ish) variables.");
  files = Malloc(p.NUM_THREADS * sizeof(files_t),"Files variables.");
  #ifdef PARALLEL
  threads = Malloc(p.NUM_THREADS * sizeof(pthread_t),"Thread variables.");
  #endif

  arg_post(&p, files);
  read_primers(&p);
  sort_primers(&p);
  #ifdef DEBUG
  print_primers(&p);
  #endif

  for(i = 0; i < p.NUM_THREADS; i++){
    g[i].p = &p;
    g[i].files = files;
    g[i].thread_num = i;
    #ifdef PARALLEL
    pthread_create(threads+i,NULL,process_thread,(void *) (g+i));
    #endif
  }

  #ifdef PARALLEL
  for(i = 0; i < p.NUM_THREADS; i++){
    pthread_join(threads[i],NULL);
    if(i) collect_stats(g,g+i);
  }
  #else
  process_file(g);
  #endif

  clean_primers(&p);
  free(g);
  free(files);
  #ifdef PARALLEL
  free(threads);
  #endif

  return (0);
}

static void *process_thread(void *arg){
  return process_file((globals_t*) arg);
}

